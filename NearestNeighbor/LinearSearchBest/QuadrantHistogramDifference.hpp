...
#ifndef LinearSearchBestQuadrantHistogramDifference_HPP
#define LinearSearchBestQuadrantHistogramDifference_HPP
...
class LinearSearchBestQuadrantHistogramDifference : public LinearSearchBestHistogramParent<PropertyMapType, TImage, TIterator, TImageToWrite>
{

public:
  /** Constructor. This class requires the property map, an image, and a mask. */
  LinearSearchBestQuadrantHistogramDifference(PropertyMapType propertyMap, TImage* const image, Mask* const mask) :
  ...
  typename TIterator::value_type operator()(const TIterator first, const TIterator last, typename TIterator::value_type query)
  {
    ...
    typedef HistogramGenerator<BinValueType>::QuadrantHistogramType QuadrantHistogramType;
    ...
    QuadrantHistogramType targetHistogram =
        MaskedHistogramGeneratorType::ComputeQuadrantMaskedImage1DHistogramAdaptive(
                  this->Image, queryRegion, this->MaskImage, queryRegion, quadrantHistogramProperties, false,
...
//      float ssdMatchHistogramScore = Histogram<BinValueType>::HistogramCoherence(targetHistogram, bestSSDHistogram);
      float ssdMatchHistogramScore = Histogram<BinValueType>::QuadrantHistogramDifference(targetHistogram, bestSSDHistogram);
    ...
    for(TIterator currentPatch = first; currentPatch != last; ++currentPatch)
    {
      ...
      QuadrantHistogramType testHistogram =
          MaskedHistogramGeneratorType::ComputeQuadrantMaskedImage1DHistogramAdaptive(
                      this->Image, currentRegion, this->MaskImage, queryRegion, returnQuadrantHistogramProperties,
            true, this->MaskImage->GetValidValue(), returnQuadrantHistogramProperties);

      float histogramDifference = HistogramDifferences::QuadrantHistogramDifference(targetHistogram, testHistogram);
