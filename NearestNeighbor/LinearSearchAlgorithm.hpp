...
inline TVertexDescriptor LinearSearchPropertyBest(
                                     TIterator first,
                                     TIterator last,
                                     TDistanceFunction distanceFunction,
                                     TPropertyMap propertyMap,
                                     TVertexDescriptor query,
                                     TCompareFunction compare = TCompareFunction(),
                                     TDistanceValue inf = std::numeric_limits<TDistanceValue>::infinity())
{
  if(first == last)
  {
    return *last;
  ...
  TDistanceValue d_best = inf;
  TIterator result = last;
  for(; first != last; ++first)
  {
    TDistanceValue d = distanceFunction(get(propertyMap, *first), get(propertyMap, query));
    if(compare(d, d_best))
    {
      d_best = d;
      result = first;
