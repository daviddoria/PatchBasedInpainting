...
#ifndef ImagePatchVectorizedIndices_H
#define ImagePatchVectorizedIndices_H
...
public:
  ...
  ImagePatchVectorizedIndices();

  /** Construct a patch from a region. Ideally 'image' would be const, but we also need a default constructor.*/
  ImagePatchVectorizedIndices(TImage* const image, Mask* const maskImage, const itk::ImageRegion<2>& region);
  ...
  const std::vector<itk::Index<2> >& GetIndexVector() const {return this->IndexVector;}

  /** Get the valid offsets of the patch. */
  const std::vector<unsigned int> * GetValidOffsetsAddress() const {return &this->ValidOffsets;}

  void CreateIndexVector();
...
#include "ImagePatchVectorizedIndices.hpp"
